    ...
    <div class="main">
        ...
        <el-card class="tagCard">
            <div class="selectCenter">
                <div class="selectContainer" :class="{fold:isFold}">
                    <el-row class="selectList" v-for="(item,index) in tagListStore.lineClassTag" :key="index">
        ...
        <div class="card_content_wrapper">
            <div class="loadMoreList">
                <div class="content_card_item" v-for="(videoModule,index) in tagListStore.lineclassData" :key="index">
                    <!-- 视频内容 -->
                    <div class="card_item">
                        <!-- 视频图片 -->
                        <div class="card-top">
...
import {reqVideoClass} from '../../../api/course/course';
// 定义展开收起的状态
...
const tagHandler = async (dances:any, dancesList:any) => {
    ...
    let res = await reqVideoClass();
    res.data.content.forEach((item:any)=>{
        tagListStore.lineclassData.splice(0,'',item)
    })
}

// 重置按钮点击回调
const resetHandler = () => {
    tagListStore.lineClassTag.forEach((item) => {

        item.choises.forEach((dance:any) => {
            dance.isClick = false;
        })
    })
    tagListStore.getLineClassdata();
}
onMounted(() => {
    tagListStore.getLineTag()
    // 通知pinia发请求 获取视频模块数据
    tagListStore.getLineClassdata()
})
...
const More = async()=>{
    ...
    result.data.content.forEach((item:any)=>{
        tagListStore.lineclassData.push(item)
    })
}
